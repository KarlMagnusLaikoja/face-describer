...
class SkinColourDescriber:
    ...
    def describeSkinColour(self, coordinates):
        ...
        #Crop the image using the cheek area coordinates and go through every pixel, return the average RGB value and corresponding skin colour
        ...
        return (r, g, b), self.compareRGB( (r, g, b) )
    ...
    def compareRGB(self, colours):
        ...
        differenceFromPaleWhite = math.sqrt(
            math.pow(colours[0]-245, 2) +
            math.pow(colours[1]-186, 2) +
            math.pow(colours[2]-164, 2)
        )
        differenceFromFair = math.sqrt(
            math.pow(colours[0]-148, 2) +
            math.pow(colours[1]-115, 2) +
            math.pow(colours[2]-106, 2)
        )
        differenceFromDarkerWhite = math.sqrt(
            math.pow(colours[0]-146, 2) +
            math.pow(colours[1]-101, 2) +
            math.pow(colours[2]-78, 2)
        )
        differenceFromLightBrown = math.sqrt(
            math.pow(colours[0]-159, 2) +
            math.pow(colours[1]-122, 2) +
            math.pow(colours[2]-110, 2)
        )
        differenceFromBrown = math.sqrt(
            math.pow(colours[0]-160, 2) +
            math.pow(colours[1]-112, 2) +
            math.pow(colours[2]-109, 2)
        )
        differenceFromDarkBrownOrBlack = math.sqrt(
            math.pow(colours[0]-50, 2) +
            math.pow(colours[1]-37, 2) +
            math.pow(colours[2]-32, 2)
        )
